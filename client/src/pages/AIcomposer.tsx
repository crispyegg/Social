...
import { ArrowRightIcon, Calendar, Clock, HistoryIcon, Loader2Icon, TimerIcon, Wand2Icon, XIcon } from "lucide-react";
...
const AIcomposer = () => {
  ...
  const [selelctedPlatforms ,setSelectedPlatforms] = useState<string[]>([])
  ...
  const fetchGeneration = async () => {
   try {
    ...
    setGenerations(data)
     ...
     toast.error(error?.response?.message?.data?.message || error?.message)
   }
  }
  ...
  const handleSchedule = async () => {
   ...
   if(selelctedPlatforms.length === 0){
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
              ...
              <div className="relative group mt-12 ">
                <textarea className="w-full px-6 py-6 bg-white border border-slate-400 outline-none focus:border-slate-400 transition resize-none h-40" placeholder="Share Your idea...(e.g A post about the launch of our new eco-frindly coffee beans" value={prompt} onChange={(e)=>setPrompt(e.target.value)}/>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">

            {
              generations.map((gen:any)=>(
                <div key={gen._id} className="group bg-white rounded-2xl border border-slate-200 p-5 hover:border-red-200 transition-all relative overflow-hidden">
...

            {
              generations.length === 0 && (
                <div className="col-span-full py-20 text-center space-y-2">
                  <div className="size-12 bg-slate rounded-2xl flex items-center justify-center mx-auto text-slate-300">
               ...
               <div className="space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {PLATFORMS.map((p)=>{
                         const active = selelctedPlatforms.includes(p.id);
                         return (
                          <button className={`p-2.5 rounded-md border text-xs ${active ?'bg-red-500/80 text-white':"bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`} key={p.id} onClick={()=> setSelectedPlatforms((prev)=>(prev.includes(p.id)? prev.filter((x)=>x !== p.id):[...prev ,p.id]))}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="relative">
                    <Calendar className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/>

                    <input type="date" className="w-full pl-11 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all"  value={scheduledDate} onChange={(e)=>setScheduledDate(e.target.value)}/>
                  </div>

                  <div className="relative">
                    < Clock  className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/>

                    <input type="time" className="w-full pl-11 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all"  value={scheduledTime} onChange={(e)=>setScheduledTime(e.target.value)}/>
                  </div>

                </div>
               </div>

                <button onClick={handleSchedule} className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-700 hover:bg-red-500 hover:text-white transition">
