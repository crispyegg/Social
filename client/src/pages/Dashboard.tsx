   ...
   const fetchDashboardData = async () => {
    try {
     ...
     setActivites(activityRes.data)
        ...
        console.error(" Error fetching dashbored data",error)  
    }
   }
   fetchDashboardData();
 },[])
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {
          statCards.map((card)=>(
            <div key={card.label} className="bg-white hover:bg-red-50 relative border border-slate-200 rounded-2xl p-5 hover:border-red-300 transition-all">
               <div className="flex items-center justify-between mb-4">
                   <div className="text-3xl font-medium text-slate-800 tabular-nums">
                      {card.value}
                   </div>
                   <div className="text-xs absolute right-4 text-red-500 flex items-center gap-1">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100" >
             ...
             <span className="text-sm text-slate-400">{activites.length}</span>
        </div>
        {
          activites.length === 0 ?(
           <div className="flex flex-col items-center justify-center py-16 px-6">
            <div className="size-12 bg-slate-100 rounded-xl flex items-center justify-center mb-3">
             <ActivityIcon className="size-6 text-slate-400"/> 
                ...
                <p className="text-slate-400 text-sm mt-1">Connect accounts and schedule post to see events here</p>  
           </div>
