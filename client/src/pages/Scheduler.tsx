       ...
       <div className="w-full lg:w-[460px] shrink-0">
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
              ...
              <form className="space-y-5" onSubmit={handleScheduled}>
                    ...
                    <label className="block text-xs text-slate-500 uppercase mb-2">Platform</label>
                     <div className="flex flex-wrap gap-3">
                         {PLATFORMS.map((p)=>{
                          ...
                          return (
                            <button key={p.id} type="button" onClick={()=>togglePlatform(p.id)} className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active?"bg-red-400 border-red-500 text-green-500 scale-103":'border-slate-200 text-slate-500 hover:border-slate-300'}`} >
                 ...
                 <div>
                  <label className="block text-xs text-slate-500 uppercasemb-2" >Media(optional)</label>
                  {mediaFile ? (
                    <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                      ...
                      <button type="button" onClick={()=>setMediaFile(null)} className="absolute top-2 right-2 size-7 bg-slate-900/60 hover:bg-slate-800/80 text-white rounded-full flex items-center justify-center transition-colors">
                 ...
                 {/*Date and Time */}
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                     ...
                     <div>
                      <label className="block text-xs text-slate-500 uppercase mb-2">Timings</label>
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
         ...
         <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
           <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100" >
            ...
            <h3 className="text-slate-900">Upcoming</h3>
                  ...
                  <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                          {post.mediaType && <span className="text-xs bg-slate-100 text-slate-600 border border-salte-200 px-1.5 py-0.5 rounded-md font-semibold capitalize">{post.mediaType}</span>}
         ...
         <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
           <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100" >
            ...
            <h3 className="text-slate-900">Published</h3>
