...
const getOrCreateZernioProfile = async (user:any) :Promise<string> => {
  try {
    ...
    if(!pid){
      throw new Error("Failed to created zernio id")
    }

    await User.findByIdAndUpdate(user._id,{zernioProfileID:pid});
    return pid;
  } catch (error:any) {
    console.error(" getorCreatedZernioProfile Error", error?.message||error);
    throw error;
  }
}
...
export const generateAuthUrl =async (req:Authrequest,res:Response):Promise<void> => {
  try {
    ...
    if(!authUrl){
      throw new Error(`Zernio returned no authUrl. Full respose:${JSON.stringify(data,null,2)}`)
    }
...
//Sync connected accounrs from zernio into mongoDb
    ...
    for(const zAccount of zernioAccounts){
      ...
      const account = await Account.findOneAndUpdate(
        {zernioAccountId:zid},{
          ...
          handle:zAccount.username || zAccount.name || "Unknown",
          zernioAccountId:zid,
          status:"connected",
